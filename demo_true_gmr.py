...
"""
# Demonstration of True Gaussian Mixture Regression (GMR)
...
predicted_ytest_all = model.predict_true(autoscaled_variables_test[:, numbers_of_x], numbers_of_x, numbers_of_y)

# Inverse analysis
estimated_x = model.predict_true(autoscaled_variables_test[:, numbers_of_y], numbers_of_y, numbers_of_x)
...
estimated_X_test = estimated_x * np.matlib.repmat(variables_train[:, numbers_of_x].std(ddof=1, axis=0), estimated_x.shape[0], 1) + \
                   np.matlib.repmat(variables_train[:, numbers_of_x].mean(axis=0), estimated_x.shape[0], 1)
