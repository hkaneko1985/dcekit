# 材料類似度研究 Phase 4：データセット横断統合解析と論文化判断
...
Phase 1のStarrydata、Phase 2のHTEM、Phase 3のNanoMine、およびPNCExtract補助解析を統合した。独立トラックの延べ試料数は **8,827** である。ただし、これは異なるデータベースを単純合計した数であり、一つの重複除去済みコーパスではない。
...
|---|---|---:|---:|---|
| Starrydata | 無機材料文献横断 | 5,001 | 947 SID | 組成、形態、合成・製造工程 |
| HTEM | 無機薄膜ライブラリ | 1,891 | 17 study連結成分 | 組成、原料・投入、雰囲気、温度・時間、基板・幾何 |
| NanoMine | ポリマーナノコンポジット | 832 | 119 論文 | matrix、filler、界面処理、loading、工程種類・系列・設定 |
| PNCExtract | 組成幅広さ補助集合 | 1,103 | 217 論文 | matrix、filler、mass/volume loading |
...
- Starrydata：F2−F0の対数共所属lift差は3 seed中央値 **0.809**、正方向率 **100.0%**。
- HTEM：工程を含むreported 650ビューで、source補正後excessの組成基準との差は中央値 **0.038**、正方向率 **88.6%**。
- NanoMine：工程を加えたreported 576ビューで、材料のみからの論文ARI差は中央値 **0.072**、正方向率 **97.6%**。
- PNCExtract：strict identityへloadingを加えると、k=50の論文ARIは **0.185 → 0.138** と低下したが、局所MRRは **0.276 → 0.582** と上昇した。

正方向率は重みグリッド内の感度集計であり、独立反復の成功確率ではない。

### 4.2 実値と欠損・記載パターンを分離する

- Starrydata：F2と欠損マスクのlift比の対数は中央値 **0.212**、工程値シャッフルとの差は **0.306** だった。
- HTEM：reportedビューの意味情報−mask対照は中央値 **0.027**。選択済み6ビューのうち **5/6** が、装置・元素系を保持した工程シャッフル95%点を上回った。
- NanoMine：設定値重視ビューのk=50論文ARIは設定値シャッフル平均より **0.044** 高かった。一方、センチネル全体の論文ARIはmaskのみより中央値 **-0.171** 低く、局所MRRは中央値 **0.103** 高かった。

工程値に信号はあるが、論文文脈の再現に関して欠損・記載様式は非常に強い。総合距離だけでなく、意味距離と記載完全度を別々に返す必要がある。

### 4.3 欠損境界は結論を動かす

HTEMではoptimistic・reported・pessimisticの全境界で工程増分が正だった設定は **29.7%** に限られ、**66.5%** では符号が変わった。NanoMineでは全境界正が **85.2%**、符号変化が **14.8%** だった。

NanoMineのoptimistic対pessimistic分割ARI中央値は **0.545**、HTEMの選択ビューと欠損境界とのARI中央値は **0.680** だった。欠損を補完して単一距離にすると、この不安定性が不可視になる。

### 4.4 大域クラスタリングと局所検索は別の問題である

HTEMでは大域文脈指標とMRRの両方が組成基準を上回ったのは650ビューの **4.6%** だけだった。大域増分と局所増分のSpearman相関は **ρ=0.417** だが、局所増分の中央値は負である。

NanoMineの6センチネルでは、大域論文ARIと局所MRRの順位相関は **ρ=0.029** で、実質的に無相関だった。均衡ビューはMRR **0.881**、工程のみは論文ARI **0.492** と、それぞれ異なる側面で強かった。
...
|---|---:|---:|---:|---:|
| Starrydata | 6 | 0.591 | 0.279 | 0.919 |
| HTEM | 15 | 0.519 | 0.351 | 0.895 |
| NanoMine | 15 | 0.647 | 0.158 | 0.847 |

いずれも中央値は0.90未満である。単一の自然な分割が不明なのではなく、**組成系列、工程系列、実験プロトコル系列が異なる関係を表すため、分割が異なること自体が期待される**。

工程重みごとの大域増分中央値は次のとおりである。

- **HTEM:** λ=0.10: 0.009, λ=0.25: 0.020, λ=0.50: 0.050, λ=0.75: 0.062, λ=1.00: 0.069
- **NanoMine:** λ=0.25: 0.017, λ=0.50: 0.061, λ=0.75: 0.127, λ=1.00: 0.150
...
- HTEM：異study・異成膜装置で、195工程込み設定の80%以上に共所属する候補 **20対**。
- NanoMine：異論文で18センチネル条件中16以上に支持される候補 **198対**。このうち **106対** は共著者またはLocation一致という外部文脈支持を持った。
...
| 判断基準 | 結果 |
|---|---|
| A. 3領域で組成以外の情報による文脈変化が正方向 | PASS |
| B. 実値が欠損・シャッフル対照を超える領域が2以上 | PASS |
| C. 大域クラスタと局所検索の用途依存が2領域以上 | PASS |
| D. 主要ビューが同一分割へ収束しない領域が2以上 | PASS |
| E. 2領域で異sourceの合意候補を回収 | PASS |
| F. ハッシュ・コード・機械可読結果・試験を保存 | PASS |
