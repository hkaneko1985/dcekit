...
# Demonstration of kNN-LOMP (for more details, please go to https://www.sciencedirect.com/science/article/pii/S2772508122000692)
...
from dcekit.variable_selection import lomp

# setting
n_virtual_samples = 100  # number of virtually generated samples
rate_max_min = 0.01  # rate of std. for half range around x
k_in_knn = 10
...
print('LOMP calculation for training data')
lomp_train = lomp(
    model,
    autoscaled_x_train,
    autoscaled_x_train.std(axis=0, ddof=1) * rate_max_min,
    n_virtual_samples=n_virtual_samples,
    random_state=None,
)
#print('LOMP calculation for test data')
#lomp_test = lomp(
#    model,
#    autoscaled_x_test,
#    autoscaled_x_train.std(axis=0, ddof=1) * rate_max_min,
...
# calculate kNN-LOMP for training data
...
for sample_number in range(autoscaled_x_train.shape[0]):
    knn_shap_train[sample_number, :] = (lomp_train[knn_index_train[sample_number, :], :].mean(axis=0))
# calculate kNN-LOMP for test data
...
for sample_number in range(autoscaled_x_test.shape[0]):
    knn_shap_test[sample_number, :] = (lomp_train[knn_index_test[sample_number, :], :].mean(axis=0))
